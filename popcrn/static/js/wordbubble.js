(function() {
  ...
  Bubbles = function() {
    ...
    update = function() {
      data.forEach(function(d, i) {
        return d.forceR = Math.max(minCollisionRadius, Math.abs(rScale(rValue(d))));
      });
    ...
    updateNodes = function() {
      ...
      return node.enter().append("a").attr("class", "bubble-node").attr("xlink:href", function(d) {
        ...
        return Math.abs(rScale(rValue(d)));
      });
    };
    updateLabels = function() {
      ...
      label.style("font-size", function(d) {
        ...
        return Math.abs(2.5 * rScale(rValue(d))) + "px";
      });
      label.append("span").text(function(d) {
        return textValue(d);
      }).each(function(d) {
        return d.dx = Math.max(Math.abs(2.5 * rScale(rValue(d))), this.getBoundingClientRect().width);
      }).remove();
  ...
  $(function() {
    ...
    d3.select("#topic-title").html(text.name);
